...
export default new class CategoryController extends Controller {
    ...
    index = async (req: Request, res: Response) => {
        try {
            ...
            if (!categories || categories.length == 0) return res._error(STATUS.NOT_FOUND, "category not found");
            const sned = CategoryModel.clientresponce((categories || []) as Category[]);
            return res._success(STATUS.OK, "Categories retrieved successfully", sned);
        } catch (e) {
            return res._error(
                STATUS.INTERNAL_SERVER_ERROR,
                e instanceof Error ? e.message : "Some error occurred"
            );
        }
    };
            ...
            if (file) {
                ...
                imageProcessing.on('message', async (result) => {
                    if (result.success) {
                        ...
                        const sliderx = await CategoryModel.find(Number(id)) as Category;
                        sliderx.image = result.name;
                        categoryCache.set(sliderx);
                    }
                });
