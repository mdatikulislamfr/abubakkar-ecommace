...
export default new class AppController extends Controller {
    index = async (_, res) => {
        try {
            ...
            if (!app)
                return res._error(STATUS.NOT_FOUND, "App data not found", AppModel.emptydata);
    ...
    set = async (req, res) => {
        try {
            ...
            if (Object.keys(app).length == 0)
                return res._error(STATUS.BAD_REQUEST, "app all data empty!");
            let idx = null;
            ...
            if (!app)
                return res._error(STATUS.NOT_FOUND, "App data not found");
    ...
    bannaerget = async (_, res) => {
        try {
            const banners = bannerCache.getAll();
            if (banners.length == 0)
                return res._error(STATUS.NOT_FOUND, "banner not found");
    ...
    bannaeradd = async (req, res) => {
        try {
            ...
            return res._error(STATUS.INTERNAL_SERVER_ERROR, e.message || "some error");
        }
    };
    update = async (req, res) => {
        try {
            ...
            const bannerItem = await BannerModel.find(Number(id));
