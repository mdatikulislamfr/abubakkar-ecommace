...
export async function createProduct(product: Product): Promise<Product> {
    try {
        ...
        // checking
        if (haveSlug) throw (`${slug} alredy have!`);
        if (!haveCategory) throw ("Category not found!");
        if (!haveBrand) throw ("Brand not found!");
        if (haveSku) throw (`SKU '${product.sku}' already exists!`);
        // create product
